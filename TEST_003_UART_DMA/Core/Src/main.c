...
#define MAX_BUF 100
	...
	int GetBuffer(char *bb)
	{
		...
		if(head == tail) return 0;	//data receive 0
		if(head < tail)				//forward
		{
			memcpy(bb, Buf + head, tail - head);	//out, start point, finish point)
			ret = tail - head;
		}
		else	//head > tail backward
		{
